...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      switchuvm(p);
      p->state = RUNNING;
      swtch(&cpu->scheduler, proc->context);//context swtch. proc->context runs
      switchkvm();//switch to the correct kernel virtual memory.
...
int putinQ(struct proc *prc){

    acquire(&ptable.lock);

    if(prc->priority == 0){
	addtoq(&ptable.pReadyList[0], prc);
	release(&ptable.lock);
	return 0;
    }
    else if (prc->priority == 1){
	addtoq(&ptable.pReadyList[1], prc);
	release(&ptable.lock);
	return 0;
    }
    else if(prc->priority == 2){
	addtoq(&ptable.pReadyList[2], prc);
	release(&ptable.lock);
	return 0;
    }
...
int addtoq(struct proc **p, struct proc *prc){
    if(!*p)
	*p=prc;
    else
    {
